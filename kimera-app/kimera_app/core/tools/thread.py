...
def funcion_by_decorator(*args, **kwargs):
...
def process_multi_thread_basic():
    ...
    future = executor.submit(funcion_basic, (2), **{'name': 'functionI', 'seconds': 3})
    future = executor.submit(funcion_basic, (9), **{'name': 'functionII', 'seconds': 9})
    future = executor.submit(funcion_basic, (1), **{'name': 'functionIII', 'seconds': 1})
...
def process_multi_thread_by_decorator():
    ...
    future = funcion_by_decorator(name='functionI', seconds=3)
    future = funcion_by_decorator(name='functionII', seconds=9)
    future = funcion_by_decorator(name='functionIII', seconds=1)
    future = funcion_by_decorator(name='functionIV', seconds=2)
    print("All Tasks complete")
